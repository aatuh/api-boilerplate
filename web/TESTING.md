...
- Mocking HTTP: use `createMockHttpClient` from `@api-boilerplate/http/testing` or pass `fetchImpl` to `createHttpClient`.
...
## Notes
- Abort/timeout-safe requests are in `packages/core/http/src/client.ts` (timeouts, retries, problem+json parsing) to keep tests deterministic.
