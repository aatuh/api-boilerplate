import {
  ...
  type Locale,
} from "@api-boilerplate/i18n-shared/config";
import { DICTIONARY_LOCALES } from "./dictionaries/registry";

const dictionaryLocales = [...DICTIONARY_LOCALES] as string[];
const envSupported = parseSupportedLocales(process.env.NEXT_PUBLIC_SUPPORTED_LOCALES);
...
if (process.env.NODE_ENV !== "production" && filtered.length < envSupported.length) {
  const unsupported = envSupported.filter((loc) => !dictionaryLocales.includes(loc));
  if (unsupported.length > 0) {
    console.warn(
      `[i18n] Ignoring unsupported locales (${unsupported.join(", ")}). Add dictionaries before enabling them.`
    );
  }
}
...
export function resolveLocale(input?: string | null, fallback: Locale = FALLBACK_LOCALE): Locale {
  return resolveLocaleShared(input, fallback, SUPPORTED_LOCALES);
}

export function normalizeLocale(input?: string | null, defaultLocale: Locale = DEFAULT_LOCALE): Locale {
...
export function resolveLocaleFromAcceptLanguage(
  ...
  return resolveLocaleFromAcceptLanguageShared(input, fallback, SUPPORTED_LOCALES);
}

export { FALLBACK_LOCALE, FALLBACK_SUPPORTED_LOCALES, LOCALE_AUTO_DETECT, parseSupportedLocales, type Locale };
