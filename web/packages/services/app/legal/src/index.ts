import type { LegalDoc, LegalTokens } from "@api-boilerplate/legal";
import {
  composeLegalDoc,
  getVanillaLegalSnippets,
  getVanillaLegalTemplate,
} from "@api-boilerplate/legal";
import {
  DEFAULT_LOCALE,
  SUPPORTED_LOCALES,
  normalizeLocale,
  type Locale,
} from "@api-boilerplate/app-i18n/config";
...
export const tokensByLocale: Record<string, LegalTokens> = {
  en: {
    ...
    SERVICE_DESCRIPTION: "API Boilerplate helps you ship Go APIs with a modern Next.js frontend.",
    CONTACT_URL: "/contact",
    GOVERNING_LAW: "Delaware",
    GOVERNING_VENUE: "Wilmington, Delaware",
    COMPANY_ADDRESS: "Wilmington, Delaware",
    SERVICE_DATA_CATEGORY: "Application data",
    SERVICE_DATA_EXAMPLES: "Account profiles, usage events, and support tickets",
    SERVICE_DATA_TYPE: "Service records",
    SERVICE_DATA_TYPE_DESCRIPTION: "Stored while your account is active and for up to 24 months after closure.",
...
export function resolveLegalLocale(input?: string | null): Locale {
  ...
  return getVanillaLegalTemplate(DEFAULT_LOCALE, "terms") ? DEFAULT_LOCALE : "en";
}
...
export function getLegalDoc(
  ...
  const meta = baseMetaByLocale[resolvedLocale] ?? baseMetaByLocale.en;
  const template =
...
export function getLegalLocaleCoverage() {
  ...
  SUPPORTED_LOCALES.forEach((locale) => {
    const available = LEGAL_SLUGS.filter((slug) => getVanillaLegalTemplate(locale, slug));
